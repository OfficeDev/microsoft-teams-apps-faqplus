...
namespace Microsoft.Teams.Apps.FAQPlusPlus.Common.Providers
{
    ...
    public class SearchServiceDataProvider : ISearchServiceDataProvider
    {
            ...
            foreach (var item in qnaDocuments)
            {
                ...
                searchEntityList.Add(
                        new AzureSearchEntity()
                        {
                            ...
                            Answer = item.Answer,
                            CreatedDate = createdDate != null ? new DateTimeOffset(new DateTime(Convert.ToInt64(createdDate.Value, CultureInfo.CurrentCulture))) : default(DateTimeOffset),
                            UpdatedDate = updatedDate != null ? new DateTimeOffset(new DateTime(Convert.ToInt64(updatedDate.Value, CultureInfo.CurrentCulture))) : default(DateTimeOffset),
        ...
        private async Task AddDatatoBlobStorageAsync(string jsonData)
        {
            ...
            CloudStorageAccount storageAccount = CloudStorageAccount.Parse(Environment.GetEnvironmentVariable(Constants.AzureWebJobsStorage));
            ...
            CloudBlockBlob blockBlob = container.GetBlockBlobReference(Constants.BlobFolderName + "/faqplusqnadata.json");
